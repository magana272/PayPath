...
function ChartTooltip({ active, payload, label, prefix = "$", formatter }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ color: "#6b6b6b", marginBottom: 4, fontSize: 10 }}>{label}</div>
      {payload.map((p, i) => (
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <AreaChart data={data} margin={{ top: 24, right: dualAxis ? 12 : 12, left: 4, bottom: xLabel ? 24 : 4 }}>
        ...
        <Legend verticalAlign="top" iconType="line" iconSize={12} wrapperStyle={{ fontSize: 10, fontFamily: "var(--font-mono)", color: "#6b6b6b", paddingBottom: 8 }} />
        {keys.map((key, i) => (
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <BarChart data={data} margin={{ top: 8, right: 12, left: 4, bottom: xLabel ? 24 : 4 }} barGap={2}>
        ...
        <Tooltip content={<ChartTooltip />} />
        {bars.length > 1 && <Legend iconType="line" iconSize={12} wrapperStyle={{ fontSize: 10, fontFamily: "var(--font-mono)", color: "#6b6b6b", paddingTop: 8 }} />}
        {bars.map((b, i) => (
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <PieChart>
        ...
        <Tooltip content={<ChartTooltip />} />
        <Legend iconType="line" iconSize={12} wrapperStyle={{ fontSize: 10, fontFamily: "var(--font-mono)", color: "#6b6b6b" }} />
      </PieChart>
    </ResponsiveContainer>
  );
});
...
export function RadialProgress({ value, max, label, color = COLORS[0], size = 80 }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
      ...
      {label && <span style={{ fontSize: 9, color: "#6b6b6b", fontFamily: "var(--font-mono)", textTransform: "uppercase", letterSpacing: "0.5px" }}>{label}</span>}
    </div>
  );
}
